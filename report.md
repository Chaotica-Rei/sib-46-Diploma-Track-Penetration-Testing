...
- будет выведено сообщение об успешной загрузке файла, при этом имя файла отобразится по шаблону "`file` + 6 случайных символов", расширение останется без изменений, в данном случае загруженный файл будет отображаться с названием `file0tl4fp.php`:
...
- далее перейдём на страницу http://92.51.39.106:7788/read?file=../../tmp/0tl4fp.php ???????????????????
