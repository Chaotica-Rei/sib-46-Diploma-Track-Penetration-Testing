...
- используя уязвимость `Path Traversal` на странице `http://92.51.39.106:7788/server.html` вводим команду `127.0.0.1 & cat /code/server.py` и нажимаем кнопку `Check`, в коде приложения в классе `UploadHandler` обнаруживаем папку, в которую сервер складывает файлы при их загрузке, это папка `/tmp`:
...
- далее, эксплуатируя уязвимость `Path Traversal`, переходим на страницу `http://92.51.39.106:7788/read?file=../../tmp/n8nb8z.py` и видим код загруженного Shell'a, однако при обращении к файлу через URL `read?file=../../` сервер только отображает содержимое файла, а не выполняет его как скрипт:
