...
- далее, эксплуатируя уязвимость `Path Traversal`, переходим на страницу `http://92.51.39.106:7788/read?file=../../tmp/n8nb8z.py` и видим код загруженного Shell'a, т.к. при обращении к файлу через URL `read?file=../../` сервер только отобразит содержимое файла, а не выполнит его как скрипт:

![](pics/ufu_shell_access.png)

Теоретически, уже имя доступ к исходным кодам приложения через уязвимость `OS Command Injection` и принимая во внимание тот факт, что приложение работает под root-правами, у потенциального злоумышленника появляется возможность модифицировать код работающего приложения так, чтобы получить возможность выполнить загруженный на сервер скрипт Web Shell для установления собственного бэкдора.
