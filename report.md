...
- http://92.51.39.106:8050 - сервис NetologyVulnApp.com, запущенный на веб-сервере Apache HTTPD 2.4.7;
- http://92.51.39.106:7788 - сервис Beemer, запущенный на веб-сервере TornadoServer 5.1.1;
...
Для сканирования хоста использовались Open Source инструменты Nmap, Dirsearch и OWASP ZAP (подробные результаты по инструменту OWASP ZAP вынесены в раздел "Тестирование", т.к. сканер позволяет не только выявить наличие уязвимости, но и одновременно протестировать её).
...
- большое количество описанных уязвимостей CVE-* для программного обеспечения, которое используется на открытых портах хоста ([детальный отчет](assets/nmap_scan_results.txt));
...
**Категория:** [A01:2021-Broken Access Control](https://owasp.org/Top10/A01_2021-Broken_Access_Control/)\
**Страница:** `http://92.51.39.106:7788/read?file=`\
**Критичность:** Высокая\
...
- перейти на страницу `http://92.51.39.106:7788/read?file=../../../../../../etc/passwd`;
...
6. **Уязвимость Unrestricted File Upload**

**Категория:** [A03:2021-Injection](https://owasp.org/Top10/A03_2021-Injection/)\
**Страница:** `http://92.51.39.106:7788/index.html`\
**Критичность:** Высокая\
**Описание:** На главной странице реализован функционал загрузки картинок. Исходя из названия `Upload Car Wallpaper` можно сказать, что к загрузке предполагаются картинки с автомобилями. Однако загружаемые файлы никак не валидируются по содержанию, в результате чего загрузить можно любой файл, например, Web Shell.\
**Рекомендация для исправления:** Валидация файлов по типу содержимого.
...
- будет выведено сообщение об успешной загрузке файла, при этом имя файла отобразится по шаблону "`file` + 6 случайных символов", расширение останется без изменений, в данном случае загруженный файл будет отображаться с названием `filen8nb8z.py`:

![](pics/ufu_shell_uploaded.png)

- т.к. сервер после загрузки не выдаёт гиперссылку на загруженный файл, то ищем его на сервере, используя уязвимость `Path Traversal` на странице `http://92.51.39.106:7788/server.html`;
- вводим команду `127.0.0.1 & cat /code/server.py` и нажимаем кнопку `Check`;
- в коде приложения в классе `UploadHandler` обнаруживаем папку, в которую сервер складывает файлы при их загрузке, это папка `/tmp`:

![](pics/ufu_uploadhandler.png)

- проверяем действительно ли эта папка содержит ранее загруженный файл, обнаруживаем наш файл в папке, но при этом конечное название файла в системе не будет содержать слово `file` в названии:

![](pics/ufu_final_name.png)

- далее, эксплуатируя уязвимость `Path Traversal`, переходим на страницу `http://92.51.39.106:7788/read?file=../../tmp/n8nb8z.py` и видим код загруженного shell'a, т.к. при обращении к файлу через URL `read?file=../../` сервер только отображает содержимое файла, а не выполняет его как скрипт:

![](pics/ufu_shell_access.png)

Даже без работающего shell'a уязвимость Unrestricted File Upload успешно проэксплуатирована, т.к. на сервер загружен файл, отличный от картинки. Теоретически, уже имея доступ к исходным кодам приложения через уязвимость `OS Command Injection` и принимая во внимание тот факт, что приложение работает под `root`-правами, а загрузка файлов на сервер реализована без валидации по типу содержимого, у потенциального злоумышленника появляется возможность модифицировать код работающего приложения так, чтобы получить возможность выполнить загруженный на сервер скрипт Web Shell для установления собственного бэкдора. 
...
Подводя итоги тестирования, можно с уверенностью сказать, что хост, расположенный по адресу 92.51.39.106, содержит большое количество уязвимостей, среди которых присутствуют наиболее критичные `SQL Injection`, `OS Command Injection`, `Unrestricted File Upload`, `Denial of Service` и `Path Traversal`.

На хосте имеются 2 сервиса:
- `NetologyVulnApp` - сервис с функционалом покупки/продажи загруженных пользовательских картинок;
- `Beemer` - автомобильный блог с функционалом проверки доступности хоста по IP-адресу.

Оба сервиса позволяют загружать на сервер файлы без валидации по типу содержимого, являются уязвимыми к внедрению SQL-скриптов, используют незащищенный HTTP-протокол, а также на них включена индексация директорий.

Сервис `Beemer` работает под `root`-правами и при его компрометации потенциальный злоумышленник может получить полный контроль над системой. 

Для устранения данных уязвимостей к рекомендациям, приведённым в `Proof of Concept` к каждой эксплуатированной уязвимости в ходе этапа тестирования, можно добавить следующие практики по обеспечению общей безопасности и профилактике появления новых угроз:

- внедрение системы менеджмента ИБ (СМИБ) в соответствии с международным стандартом по ИБ - ISO/IEC 27001, для России существует национальный стандарт ГОСТ Р ИСО/МЭК 27001:2021, который фактически идентичен международному;
- внедрение механизмов безопасной разработки DevSecOps (SAST-анализ, DAST-анализ), в качестве основы можно использовать ГОСТ Р 56939-2024 "Защита информации. Разработка безопасного программного обеспечения. Общие требования";
- проведение оценки компании, разработавшей данные сервисы, по модели зрелости (например OWASP SAMM).