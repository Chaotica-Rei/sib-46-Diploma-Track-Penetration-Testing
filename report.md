...
- в открывшейся форме для загрузки картинок заполнить необходимые поля, выбрать для загрузки заранее подготовленный файл PHP-shell, нажать кнопку `Upload File`:
...
- используяю уязвимость `Path Traversal`, выявленную на предыдущем этапе, переходим на уязвимую страницу `http://92.51.39.106:8050/upload/` для просмотра содержимого папки `upload` и находим путь до загруженного PHP-shell:
